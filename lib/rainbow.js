...
export default {
  ...
  activate () {
    const pairConfig = atom.config.get('rainbow.pairs').trim()
    const quoteConfig = atom.config.get('rainbow.quotes').trim()

    this.subscriptions = new CompositeDisposable()
    this.pairs = stringToPairs(pairConfig)
    this.ends = Object.values(this.pairs)
    this.pairsRegExp = new RegExp(
      `[${escapeStringRegexp(pairConfig)}]` +
      [...quoteConfig].map(regExpIgnoreSurroundingChar).join(''),
      'g'
    )
